...
* plugins: bootstrap-transition.js, bootstrap-collapse.js
...
jQuery(document).ready(function($) {
    ...
    */
	...
	// ############ MENU ITEMS ###############
	  ...
	  $(".touch .globalnav > ul > li > a").click(function(e) {
	    if($(this).siblings().size() > 0){
	    
			e.preventDefault();
	    
		    // first looking to see if any sibling menus are expanded...then shut those 
			$(this).parent().siblings().find('.expanded').removeClass('expanded').fadeOut("fast");
			  
		    // looking to see if the current menu is open, if so shut it, if not...open it 
		    if($(this).parent().find('ul.sub-menu').hasClass('expanded')) { 
		    
			    $(this).parent().find('ul.sub-menu').removeClass('expanded').fadeOut("fast"); 
			    
		    } else {
		    
		    	$(this).parent().find('ul.sub-menu').addClass('expanded').fadeIn("fast");
		    }
